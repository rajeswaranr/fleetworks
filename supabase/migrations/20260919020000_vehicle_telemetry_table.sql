...
CREATE INDEX IF NOT EXISTS telemetry_geo_idx
  ON vehicle_telemetry USING GIST (ll_to_earth(latitude, longitude))
...
CREATE MATERIALIZED VIEW IF NOT EXISTS vehicle_telemetry_hourly
...
CREATE MATERIALIZED VIEW IF NOT EXISTS vehicle_telemetry_daily
WITH (timescaledb.continuous) AS
SELECT
...
SELECT add_continuous_aggregate_policy('vehicle_telemetry_daily',
